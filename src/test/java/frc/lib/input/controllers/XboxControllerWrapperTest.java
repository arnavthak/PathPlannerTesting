...
public class XboxControllerWrapperTest {
  ...
  public void testSticks() {
    ...
    assertEquals(0.5, controller.getLeftX(), DELTA);
    assertEquals(-0.3, controller.getLeftY(), DELTA);
    assertEquals(-0.9, controller.getRightX(), DELTA);
  ...
  @Test
  public void testSetRumbleAnim() {
    XboxControllerWrapper controller = new XboxControllerWrapper(0);

    controller.setRumbleAnimation(new RumbleOn());

    assertTrue(controller.rumbleAnimationCommand.isScheduled());

    controller.rumbleAnimationCommand.execute();

    // Currently no way to get the rumble output

    controller.setRumbleAnimation(new RumbleOff());
  }
}
